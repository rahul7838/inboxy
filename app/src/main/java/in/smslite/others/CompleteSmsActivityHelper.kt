    ...
    fun contextualActionMode(recyclerView: RecyclerView, context: Context, activity: Activity?,
        ...
        recyclerView.addOnItemTouchListener(RecyclerItemClickListener(context, recyclerView, object : RecyclerItemClickListener.OnItemClickListener {
            override fun onItemClick(view: View?, position: Int) {
                if (isMultiSelect) {
                    timeStamp = completeSmsAdapter.smsConversation.get(position).receivedDate
                    if (view != null) {
...
//          address = smsAdapter.messages.get(position).getAddress();
                    timeStamp = listOfItem!![position].receivedDate
                    isMultiSelect = true
                    if (mActionMode == null) {
...
//      }
        ...
        override fun onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean {
            return when (item.itemId) {
                R.id.action_delete -> {
                    if (true == mContext?.let { MessageUtils.checkIfDefaultSms(it) }) {
                        alertDialog()
                    } else {
                        mContext?.let { MessageUtils.setDefaultSms(it) }
                    }
                ...
                R.id.action_select_all -> {
                    ...
                    var i = 0
                    while (i < size) {
                        selectedTimeStampList.add(CompleteSmsActivity.selectedItem.get(i).receivedDate)
                        i++
                    }
                    mActionMode!!.title = "" + CompleteSmsActivity.selectedItem.size
                    if (CompleteSmsActivity.selectedItem.size > 1) {
                ...
                R.id.action_copy -> {
                    ...
                    val body: String = CompleteSmsActivity.selectedItem.get(0).body
